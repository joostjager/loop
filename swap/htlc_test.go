...
func assertEngineExecution(t *testing.T, testNum int, valid bool,
	...
	fmt.Println(debugBuf.String())
	t.Fatalf(
		"%v spend test case #%v execution ended with: %v",
		validity, testNum, vmErr,
	)
}
...
func TestHtlcV2(t *testing.T) {
	...
	testCases := []struct {
		name    string
		witness func() (wire.TxWitness, error)
		valid   bool
	}{
		{
			// Sender can spend with valid preimage.
			"success case spend with with valid preimage",
			func() (wire.TxWitness, error) {
				sweepTx.TxIn[0].Sequence = 1
				sweepSig, err := signTx(
				...
				return htlc.GenSuccessWitness(
					sweepSig.Serialize(), testPreimage,
				)
			}, true,
		},
		{
			// Sender can't spend with the valid preimage with
			// a zero sequence.
		...
		{
			"timeout case no spend with no timeout",
			// Sender can't spend when not yet timeouted.
			func() (wire.TxWitness, error) {
		...
		{
			"timeout case others cannot spend",
			// Others can't spend after timeout.
			func() (wire.TxWitness, error) {
				sweepTx.LockTime = testCltvExpiry
				sweepSig, err := signTx(
					sweepTx, receiverPubKey, receiverSigner,
	...
	for idx, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			sweepTx.TxIn[0].Witness, err = testCase.witness()
			require.NoError(t, err)
			...
			assertEngineExecution(t, idx, testCase.valid, newEngine)
		})
	}
}
